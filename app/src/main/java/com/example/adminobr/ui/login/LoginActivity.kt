...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val savedEmpresaName = sharedPrefs.getString("empresaName", null)
        Log.d("LoginActivity", "Loaded userName: $savedUserName, empresaName: $savedEmpresaName")

        if (savedUserName != null && savedEmpresaName != null) {
            binding.usuarioEditText.setText(savedUserName)
            binding.empresaAutocomplete.setText(savedEmpresaName)

            // Obtener la instancia de Empresa utilizando autocompleteManager
            selectedEmpresa = autocompleteManager.getEmpresaByName(savedEmpresaName)
        }
...
//        val isDebuggable = (applicationInfo.flags and ApplicationInfo.FLAG_DEBUGGABLE) != 0
    ...
    private fun handleLoginResponse(response: Response<LoginResponse>) {
        if (response.isSuccessful) {
            ...
            Log.d("LoginActivity", "Login response: ${Gson().toJson(loginResponse)}")
            ...
            if (loginResponse != null && loginResponse.success) {
                ...
                // Guardar los datos de inicio de sesion en SharedPreferences
                val sharedPrefs = getSharedPreferences("login_prefs", MODE_PRIVATE)
                with (sharedPrefs.edit()) {
                    Log.d("LoginActivity", "Saving userName: ${loginResponse.user.legajo}, empresaName: ${selectedEmpresa?.nombre}")
                    putString("userName", loginResponse.user.legajo)
                    putString("empresaName", selectedEmpresa?.nombre)
                    apply()
                }
